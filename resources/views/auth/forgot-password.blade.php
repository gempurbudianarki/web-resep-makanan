...
<head>
    ...
    <title>Lupa Password · BagiResep</title>
    ...
    <div class="w-full max-w-sm slide-up">
        <div class="bg-white rounded-3xl shadow-2xl p-6 lg:p-8">
            <div class="text-center mb-6">
                ...
                <h2 class="text-xl font-display font-bold text-charcoal-500">{{ __('ui.lupa_password_title') }}</h2>
                <p class="text-gray-400 text-xs mt-1">{{ __('ui.lupa_password_desc') }}</p>
            </div>

            @if(session('success'))
                <div class="bg-olive-50 border border-olive-200 text-olive-700 px-3 py-2 rounded-xl mb-4 text-xs text-center">
                    {{ session('success') }}
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('password.email') }}" class="space-y-3">
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" required autofocus
                           class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm text-charcoal-500 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent transition-all duration-300" placeholder="nama@email.com">
                </div>
                <button type="submit" class="w-full bg-amber-400 hover:bg-amber-500 text-white font-bold py-2.5 rounded-xl transition-all duration-300 shadow-md hover:shadow-lg hover:-translate-y-0.5 active:scale-[0.98] mt-1">
                    {{ __('ui.kirim_link_reset') }}
                </button>
            </form>

            <div class="mt-5 text-center">
                <p class="text-xs text-gray-400">{{ __('ui.ingat_password') }} <a href="{{ route('login') }}" class="text-amber-500 hover:text-amber-600 font-semibold transition-colors">{{ __('ui.masuk') }}</a></p>
            </div>
        </div>
